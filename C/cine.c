...
int borrarPeliculaSala(sqlite3 *db,int tamanyoCines,struct Cine *cines){
    ...
    while(fin2!=1){
        ...
        if(cont==0){
            ...
            printf("Seleccione el plazo horario que quiere eliminar o q para volver:\n");
            fflush(stdout);
            fgets(str,MaxNum,stdin);
            if(str[0]=='q'){
                return 0;
...
int crearFichero(sqlite3 *db,int tamanyoCines,int maxSala,struct Cine *cines){
	...
	while(str[0]!='q'){
		opc=0;
		printf("1.Introduce contrasenya: \n");
		printf("q.para salir\n");
        fflush(stdout);
		fgets(str,MaxLine,stdin);
		if(str[0]=='q'){
			...
			char *str_fmt = malloc((MaxLine) * sizeof(char));
	        sscanf(str, "%s", str_fmt); 
	        int tamanyo = strlen(str_fmt);
	        contrasenya= malloc((tamanyo + 1) * sizeof(char));
            strcpy(contrasenya, str_fmt);
	        free(str_fmt);
            printf("Introduce el puerto: o q para salir \n");
            fgets(str,MaxLine,stdin);
            fflush(stdout);
		    if(str[0]=='q'){
                return 0;
	            ...
	            sscanf(str, "%s", str_fmt); 
	            int tamanyo = strlen(str_fmt);
	            puerto= malloc((tamanyo + 1) * sizeof(char));
                strcpy(puerto, str_fmt);
	            free(str_fmt);
            }
